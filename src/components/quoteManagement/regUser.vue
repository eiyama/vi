<template>
  <div class="customer">
    <mt-header fixed class="def-header cust-head" title="注册">
      ...
      <div class="mint-region">
        ...
        <div v-show="hasReferrer||hasReferrerMobile" class="boxClass">
          ...
          <mt-field class="mint-formItm" label="手机号" v-show="!hasReferrerMobile">
            ...
            <!-- <div style="position: absolute;right: 0;">
              <mt-button @click="getVerifyCode"  :disabled="disabled" type="default">{{btnTitle}}</mt-button>
            </div>  -->
          </mt-field>
        </div>
        <div v-show="hasSignature" class="boxClass">
          <hr width="100%" color="#f5f5f5" size="1" />
          <p class="tip" style="line-height:30px">手写签名
            <van-button icon="edit" style="float:right;color:#0070D3;font-size:12px" type="default" size="small" @click="showSignature=true" plain>签名</van-button>
          </p>
        </div>
        <div style="padding-left:10px;background-color:#FFF;min-height:45px;display: flex;align-items:center;" v-show="hasAgreements">
          ...
          <div style="float:left;" v-for="item in agreements">
            <span style="color:#0070D3;margin-left:10px" @click="openAgreement(item)">
              << {{item.name}}>>
            </span>
          </div>
        </div>
          ...
          <div class="messagecc">
            <mt-header class="def-header" title="手写签名">
              ...
              <div slot="right" @click="saveSignature">
                <van-button style="float:right;color:#0070D3;font-size:12px" type="default" size="small" plain>确认</van-button>
              </div>
            </mt-header>
...
export default {
  ...
  data() {
    return {
      ...
      education: '',
      educationVal: '',
      educationSlots: [{ flex: 1, defaultIndex: 0, values: ["初中", "高中", "大专", "本科", "硕士", "博士", "其他"], className: 'slot1' }],
  ...
  methods: {
    ...
    selectTypeVal: function() {
      ...
      // console.log( this.ownerIdTypeVal[0],this.ownerIdTypeVal[1]);
