  ...
  <div class="inslist" ref="inslist" style="background-color:#F5F5F5;">
    <mt-header fixed class="def-header cust-head" title="报价列表">
      ...
      <div slot="right">
        <van-checkbox v-model="checked" style="float:right">
          <img slot="icon" slot-scope="props" :src="props.checked ? icon.active : icon.inactive" style="width:24px;height:24px;">
        </van-checkbox>
      </div>
    </mt-header>
    <div class="container-center">
      ...
      <div :style="inwrppera" ref="wrppera">
        <div class="mint-region" v-for="(item,index) in insArr">
          <div @click.stop="toQuoteDetail(item,index)">
            <mt-cell class="mint-list" title="" :title="item.insCorp.shortName" icon="back">
              ...
              <span class="quote-money" style="display: inline-grid;" v-show="item.insureStatus != 'loading'">
                <p style="padding-bottom: 7px;text-align:right" v-show="item.insureStatus != 'QuotedFailed' && item.insureStatus != 'VerifiedFailed' &&  item.insureStatus != 'Duplicate' " >￥{{item.premium}}</p>
                <p style="color:#4192E8;margin-left:12px;text-align:right" v-show="item.insureStatus" v-text="item.insureStatus == 'VerifiedSuccess'?'待缴费': item.insureStatus == 'QuotedSuccess'?'待核保':item.insureStatus == 'Paid'?'已支付':item.insureStatus == 'Insured'?'已承保':
          ...
          <div style="height:20px" v-show="item.insureStatus != 'QuotedFailed' && item.insureStatus != 'VerifiedFailed' 
              && item.costsDisplay && checked">
            <p style="float:right;font-size:10px;color: #0070D3;margin-right:25px">{{item.costsDisplay}}</p>
          </div>
          <div class="mint-results">
            <div class="results-area">
              ...
              <div class="msgUp">
                <van-button style="color: #4A66FE;border:1px solid #4A66FE;border-radius:5px;margin-bottom: 10px;" v-show="item.isRequireMaterial" size="small" @click="uploadAttachment(item.insBatchNo,item.insCorp.code)">资料补充</van-button>
                <van-button style="color: #4A66FE;border:1px solid #4A66FE;border-radius:5px;margin-bottom: 10px;" v-show="item.insureStatus == 'VerifiedSuccessToShuanglu'" size="small" @click="goDualRecord(item,index)">去双录</van-button>
                <van-button style="color: #4A66FE;border:1px solid #4A66FE;border-radius:5px;margin-bottom: 10px;" v-show="item.insureStatus == 'QuotedFailed' || item.insureStatus == 'VerifiedFailed'" size="small" @click="modifyInfo(item,index)">修改资料</van-button>
                <van-button style="color: #4A66FE;border:1px solid #4A66FE;border-radius:5px;margin-bottom: 10px;" v-show="item.insureStatus == 'QuotedFailed' || item.insureStatus == 'VerifiedFailed'" size="small" @click.stop="reQuote" :code="item.insCorp.code">重新报价</van-button>
                <van-button style="color: #4A66FE;border:1px solid #4A66FE;border-radius:5px;margin-bottom: 10px;" v-if="((item.insureStatus == 'QuotedSuccess' && item.bsRiskKind) || (item.insureStatus == 'QuotedFailed' && item.canModifyDiscount)) && (item.insCorp.code != 'pingan'  ) " size="small" @click.stop="openModifyInDiscount(item)" :code="item.insCorp.code">申请商业险折扣</van-button>
              </div>
            </div>
          </div>
        </div>
    ...
    <van-popup v-model="showfullPrice" position="bottom">
      <mt-header class="def-header" title="车型不匹配">
        ...
        <div slot="right" style="display:flex;height:30px;justify-content: flex-end;">
          <van-button style="color:#4A66FE;font-size:14px;" type="default" size="small" @click="reSelectQuote" plain>重新报价</van-button>
        </div>
      </mt-header>
    ...
    <van-popup v-model="showInsuredInfo" position="bottom">
      <mt-header class="def-header" title="险种投保日期有误">
        ...
        <div slot="right" style="display:flex;height:30px;justify-content: flex-end;">
          <van-button style="color:#4A66FE;font-size:14px;" type="default" size="small" @click="reInsured" plain>重新报价</van-button>
        </div>
      </mt-header>
    ...
    <van-popup v-model="showDiscountSchemes" position="bottom">
      <mt-header class="def-header" title="平安-选择方案决策">
        ...
        <div slot="right" style="display:flex;height:30px;justify-content: flex-end;">
          <van-button style="color:#4A66FE;font-size:14px;" type="default" size="small" @click="reSelectScheme" plain>重新报价</van-button>
        </div>
      </mt-header>
      <table class="mint-table" style="margin-top:-1px;" cellpadding="0" cellspacing="0">
        <colgroup>
    ...
    <van-popup v-model="showModifyInsDiscount" position="bottom">
      <mt-header class="def-header" title="商业险折扣申请">
        ...
        <div slot="right" style="display:flex;height:30px;justify-content: flex-end;">
          <van-button style="color:#4A66FE;font-size:14px;" type="default" size="small" @click="reModifyInDiscount" plain>重新报价</van-button>
        </div>
      </mt-header>
