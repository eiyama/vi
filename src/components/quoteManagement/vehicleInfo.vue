<template>
  <div class="vehicleInfo" style="background-color:#FFF">
    ...
    <div class="quote-row">
      以下信息请根据行驶证填写
      <mt-button v-show="showPolicy" style="float:right;font-size:13px;margin-left: 10px;height:25px;width:80px;line-height: 25px;" plain type="primary" @click="viewPolicy()">往年续保</mt-button>
      <div v-show="showOcr && canUseOcr" v-showAppFeatures="['ORC']" style="float: right;display: flex;align-items: center;">   
          <!-- v-showAppFeatures="['ORC']" -->
          <span style="font-size:12px;color:#333;margin-right:8px;">行驶证识别</span>     
          <img  @click="showOccre" style="height: 30px;" src="../../assets/images/icon-Photo.png" />          
      </div>      
    </div>
    <div class="mint-region">
      <hr width="100%" color="#f5f5f5" size="1">
      ...
      <mt-field label="车架号" class="upper" type="text" placeholder="请输入车架号" v-model="carInfoData.car.vin">
        <div style="display:flex">
              <van-button size="small" style="color:#4A66FE;border:1px solid #4A66FE;border-radius:8px;" plain @click="openPriceModelBoxByVin"> 查车型</van-button>
              <van-button size="small" style="color:#4A66FE;border:1px solid #4A66FE;margin-left:5px;border-radius:8px;" plain v-if="isQueryVehicle" @click="queryVehicle">查交管</van-button>  
        </div>        
      </mt-field>
      <hr width="100%" color="#f5f5f5" size="1">
      ...
      <mt-field label="品牌型号" type="text" placeholder="请输入品牌型号" v-model="carInfoData.car.model">
        <van-button size="small" style="color:#4A66FE;border:1px solid #4A66FE;margin-left:5px;border-radius:8px" plain @click="openPriceModelBoxBymodel">查车型</van-button>
      </mt-field>
      <hr width="100%" color="#f5f5f5" size="1">
      <div class="mint-region">
    ...
    <div class="mint-region">
      ...
      <div style="display: flex;align-items: center;">
        ...
        <van-button v-show="carInfoData.car.owner.relationType != 'Person'" size="small" style="margin-right: 15px;color:#4A66FE;border:1px solid #4A66FE;margin-left:5px;border-radius:8px" plain @click="showSeachOrgCode(carInfoData.car.owner.name,carInfoData.car.owner.idNo,'car')">机构代码查询</van-button>
    ...
    <div class="mint-region">
      ...
      <div v-show="!isApplicantSameOwner">
        ...
        <div style="display: flex;align-items: center;">
          ...
          <van-button v-show="carInfoData.applicant.relationType != 'Person'" size="small" style="margin-right: 15px;color:#4A66FE;border:1px solid #4A66FE;margin-left:5px;border-radius:8px" plain @click="showSeachOrgCode(carInfoData.applicant.name,carInfoData.applicant.idNo,'applicant') ">机构代码查询</van-button>
        </div>
        <hr width="100%" color="#f5f5f5" size="1">
    ...
    <div class="mint-region">
      ...
      <div v-show="!isInsuredSameOwner && !isInsuredSameApplicant">
        ...
        <div style="display: flex;align-items: center;">
          ...
          <van-button v-show="carInfoData.insured.relationType != 'Person'" size="small" style="margin-right: 15px;color:#4A66FE;border:1px solid #4A66FE;margin-left:5px;border-radius:8px" plain @click="showSeachOrgCode(carInfoData.insured.name,carInfoData.insured.idNo,'insured')">机构代码查询</van-button>
        </div>
        <hr width="100%" color="#f5f5f5" size="1">
        <mt-field class="mint-formItm" label="证件号" type="text" :disabled="true">
    ...
    <van-popup v-model="messageone" position="bottom">
      <div class="messagecc">
        <mt-header class="def-header" title="交管车辆信息不一致">
          <div slot="left">
            <mt-button style="font-size:15px;color:#4A66FE" @click="closeone(false)">取消</mt-button>
          </div>
          <div slot="right">
            <mt-button style="font-size:15px;color:#4A66FE" @click="closeone(true)">更新</mt-button>
          </div>
        </mt-header>
    ...
    <mt-popup v-show="inputPlateNo" position="bottom" popup-transition="popup-fade">
      <div id="dispense">
        <mt-header fixed class="def-header" title="车牌号">
          <div slot="left">
            <mt-button style="font-size:16px;color:#4A66FE" @click="closePlanteNoPop(false)">取消</mt-button>
          </div>
          <div slot="right">
            <mt-button style="font-size:16px;color:#4A66FE" @click="closePlanteNoPop(true)">确定</mt-button>
          </div>
        </mt-header>
...
export default {
  ...
  mounted() { // 获取续保信息
    ...
    if(carInfoParams.insCityCode && carInfoParams.insCityCode.indexOf("44") -1 && carInfoParams.insCityCode.indexOf("4403") == -1){
      this.isQueryVehicle = true;
    }
  ...
  methods: {
    ...
    selectedCity: function(cityCode, cityShortName) {
      this.showCity = false;
      sessionStorage.setItem('insCityCode', cityCode);
      console.log(cityCode.indexOf("44") > -1, cityCode.indexOf("4403") < 0)
      if(cityCode && cityCode.indexOf("44") > -1 && cityCode.indexOf("4403") < 0){
        this.isQueryVehicle = true;
      }else{
        this.isQueryVehicle = false;
      }
          ...
          axiosGetAPI(url).then((res) => {
            ...
            if (res.result) {
              ...
              if (self.officialData.plateNo == self.carInfoData.car.plateNo &&
                ...
                self.messageone = true;
                self.hastrigger = true;//是否是主动查询交管信息  
              }
            } else {
              self.messageone = false;
              self.update = false;
      ...
      .content-section-flex {
        ...
        &.flex-card {
          ...
          .isNumClick {
            ...
            border: 1px solid #4A66FE !important;
            /*no*/
            z-index: 10;
