<template>
  <div class="home" style="background-color:#FFF;">
    <mt-header fixed class="def-header cust-head" title="团队关系">
      ...
      <div slot="right" v-show="iszxTeam" @click="toTeamDetail">
         <mt-button > <p style="color:#4A66FE;font-size:14px;">团队详情</p></mt-button>
      </div>
    </mt-header>
    <div style="padding-top:48px;background-color:#FFF;width:100%;height: 100%;position: absolute;">
      ...
      <div class="parentWrap" style="color:#333;font-size:12px;">
        <van-row v-show="!iszxTeam">
          <van-col span="8" style="line-height: 40px;font-size: 15px;font-weight: 600;text-align: center;border-bottom: 1px solid #eee;border-right: 1px solid #eee;">
            我的直辖
            <!-- 本群 -->
          </van-col>
          <van-col span="8" style="line-height: 40px;font-size: 15px;font-weight: 600;text-align: center;border-bottom: 1px solid #eee;border-right: 1px solid #eee;">
          ...
          <van-col span="8" style="line-height: 40px;font-size: 15px;font-weight: 600;text-align: center;border-bottom: 1px solid #eee;">
            我的二级直辖
          </van-col>
        </van-row>
         <van-row v-show="iszxTeam">
          <van-col span="12" style="line-height: 40px;font-size: 15px;font-weight: 600;text-align: center;border-bottom: 1px solid #eee;border-right: 1px solid #eee;">   
            本群（一级）
          </van-col>
          <van-col span="12" style="line-height: 40px;font-size: 15px;font-weight: 600;text-align: center;border-bottom: 1px solid #eee;border-right: 1px solid #eee;">
            子群（二级）
        ...
        <van-row v-show="!iszxTeam" style="background-color:#FFF;" type="flex" justify="end">
              ...
              <div v-for="(item,index) in personData" :key="index" style="height:45px;border:0.1px solid #F5F5F5;" @click="onLoadFirst(index, item)">
                ...
                height:100%;padding: 0 10px;justify-content: space-between;">
                  <span style="display: flex;align-items: center;">
                    ...
                    <p  @click="openInfo(item)"> {{item.name || item.linkedUser}}</p>
                  </span>
            ...
            <van-list style="width:100%">
              ...
              <div v-for="(item,index) in personSecondData" :key="index" style="height:45px;border:0.1px solid #F5F5F5;">
                <p style="display:flex;align-items:center;font-size:14px;color:#333;height:100%;padding-left:10px">
                  <van-icon name="contact" style="margin-right:5px"/>
                  <span @click="openInfo(item)">{{item.name || item.linkedUser}}</span>
                </p>
              </div>
            </van-list>
          </van-col>
        ...
        <van-row v-show="iszxTeam" style="background-color:#FFF;" type="flex" justify="end">
          <van-col span="12" style="border:0.1px solid #F3F3F3">
            <van-list v-model="loading" :finished="finished" finished-text="" @load="onLoad" style="width:100%">
              <!-- <van-cell v-for="item in personData" :key="item" :title="item" icon="location-o" is-link/> -->
              <div v-for="(item,index) in personData" :key="index" style="height:45px;border:0.1px solid #F5F5F5;" @click="onLoadFirst(index, item)">
                <p :class="{classA:currentIndex == index}" style="display:flex;align-items:center;font-size:14px;color:#333;
                height:100%;padding: 0 10px;justify-content: space-between;">
                  <span style="display: flex;align-items: center;">
                    <img  :src="item.wxHeadImg" alt="" v-if="item.wxHeadImg" style="height:28px;margin-right:5px;border-radius: 50%;">
                    <van-icon size="18" name="contact" v-else style="margin-right:5px" />
                    <p  @click="openInfo(item)"> {{item.name || item.wxName || item.linkedUser}}</p>
                  </span>
                  <van-icon name="arrow" styl
                  e="margin-left:10px"/>
                </p>
              </div>
            </van-list>
          ...
          <van-col span="12" style="border:0.1px solid #F3F3F3">
            <van-list style="width:100%">
              <!-- <van-cell v-for="item in personFirstData" :key="item" :title="item" /> -->
              <div v-for="(item,index) in personFirstData" :key="index" style="height:45px;border:0.1px solid #F5F5F5;" >
                <p :class="{classA:currentIndexB == index}" style="display:flex;align-items:center;font-size:14px;color:#333;
                height:100%;padding: 0 10px;justify-content: space-between;">
                  <span style="display: flex;align-items: center;">
                    <img  :src="item.wxHeadImg" alt="" v-if="item.wxHeadImg" style="height:28px;margin-right:5px;border-radius: 50%;">
        ...
        <mt-popup v-model="showInfo" position="middle" popup-transition="popup-fade" style="width:65%;height:250px;border-radius:5px">
          <div class="teammessagecc">
            <div style="margin-top:5px;margin-bottom:5px;padding-top:15px;padding-left:15px;padding-right:15px;text-align:center">
              <img class="member-head" :src="itemData.headImageUrl || itemData.wxHeadImg || avatar" alt="">
              ...
              <p style="font-size:13px;color:#333;margin-top:3px">联系方式: <span style="color:#666">{{itemData.linkedUser}}</span></p>
            </div>
          </div>
        </mt-popup>
        <!-- <van-popup v-model="showExplain" position="bottom" >
          <mt-header fixed class="def-header cust-head" title="车险奖励基本法">
            <div slot="left" v-show="iszxTeam" @click="showExplain = false">
...
export default {
  ...
  created: function() {
    ...
    this.iszxTeam = (appId == 'cx7190761141531rzi' || appId == 'cxa181211170855pxj');

  },
  methods: {
    toTeamDetail(){
        // this.$router.push('/myInfo/teamDetail');
    ...
    openBack: function() {
      this.$router.push('/myInfo/myIndex')
    },
