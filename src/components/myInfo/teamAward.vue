    ...
    <div class="main">
        ...
        <div class="mainItem">
              ...
              <div  class="itemList" v-for="(item,index) in outPolicyList" :key = index> 
                  ...
                  <span  class="listSecond"><p>{{item.ownerType}} - {{item.sellerName}}</p>  <p  class="level">{{item.policyBonus}} 元 </p></span> 
                  <p class="listThird">{{item.plateNo}}   {{item.insCorpName}} |  保费{{item.premium}}</p>
              </div>
              <p v-if="outPolicyList.length < 1" class="noneList">本期暂无清单数据</p>
              <!-- <div class="itemList">
  ...
  mounted: function() {
    ...
    this.mainStyle.minHeight = h + 'px';
      ...
      var groupowner = JSON.parse(sessionStorage.getItem('GroupOwner'));
                 ...
                 this.outPolicyList = groupowner.groupPolicies;
  },
  methods: {
    openBack: function() {
      window.history.back();
...
<style scoped>
    .main{padding-top: 49px;min-height:100vh;}
    .mainItem{
        margin: 10px 15px;
        border-radius: 5px;
