							 ...
							 Indicator.open('出单中...');
								hander = setInterval(function() {
									...
									if(time > 0){
										...
										// if(Data.result.status  == "IssueFailed" || Data.result.status  == "Cancel" ){
										...
										// 		}).catch(() => {}); 
										// }else{
										// 	axiosGetAPI('/PropertyProducts/FDD001/Order/' + res.result.orderNo +  '/Payment').then(data=>{
										// 		if(data.result.payUrl){
										...
										// 	})
										// }
										if(Data.result.status  == "PendingPayment" || Data.result.status  == 'Initialization' || Data.result.payUrl){
											var	payurl = Data.result.payUrl;  
											clearInterval(hander);
											Indicator.close();
											self.$router.push({path:"/zxfdPage/payPage?payurl=" + encodeURIComponent(payurl)});
										} else if(Data.result.status  == "IssueFailed" || Data.result.status  == "Cancel" ){
											clearInterval(hander);
											Indicator.close();
											// Toast('出单失败，请重新提交信息');
											self.$dialog.confirm({
